    ...
    <div className="bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid lg:grid-cols-[16rem_minmax(0,1fr)] gap-10 sm:gap-12 items-start">
          <aside className="lg:sticky lg:top-24 self-start space-y-6 w-full max-w-full">
            <div className="p-5 rounded-lg border bg-white">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-eyebrow font-semibold text-gray-900">Filter posts</h3>
                {(tag || search) && (
                  <Link href="/blog" className="inline-flex items-center text-sm text-secondary hover:opacity-80" data-event="filters_clear_all" data-location="blog">Clear</Link>
                )}
              </div>
            ...
            <div className="p-5 rounded-lg border bg-white max-w-full">
              <p className="text-eyebrow font-semibold text-gray-900">Weekly automation insights</p>
          ...
          <main className="mt-6 lg:mt-0 min-w-0">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  {featuredPosts.map((post) => (
                    <Link key={post.slug} href={post.url} className="group block rounded-xl overflow-hidden border bg-white hover:shadow-md transition-shadow">
                      ...
                      <h3 className="text-2xl font-bold text-gray-900 group-hover:text-secondary leading-tight">{post.title}</h3>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-title font-bold text-gray-900">Recent posts</h2>
              </div>
              {pagedRecent.length === 0 ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {pagedRecent.map((post) => (
                    <Link key={post.slug} href={post.url} data-event="post_click" data-location="blog-list" data-slug={post.slug} data-title={post.title} className="group block rounded-xl overflow-hidden border bg-white hover:shadow-md transition-shadow">
                      ...
                      <div className="p-5">
                        ...
                        <h3 className="text-lg font-semibold text-gray-900 group-hover:text-secondary leading-snug">{post.title}</h3>
                ...
                <nav className="mt-8 flex items-center justify-between">
                  {currentPage > 1 ? (
                    <Link
                      ...
                      className="inline-flex items-center rounded-md border bg-white px-3 py-2 text-sm text-secondary hover:bg-gray-50 hover:opacity-80"
                    >
                      ← newer
                    </Link>
                  ) : <span />}
